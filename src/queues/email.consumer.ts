...
export async function consumeAuthEmailNotification(
  channel: Channel,
): Promise<void> {
  try {
    if (!channel) {
    ...
    const jobberQueue = await setUpQueue(
      channel,
      exchangeName,
      routingKey,
      queueName,
    ...
    log.error(
      'NotificationService consumeAuthEmailNotification() method error',
      error,
    );
  }
}

export async function consumeOrderEmailNotification(
  channel: Channel,
): Promise<void> {
  try {
    if (!channel) {
