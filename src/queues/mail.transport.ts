...
const log: Logger = winstonLogger(`${config.ELASTIC_SEARCH_URL}`, 'email-transport', 'debug');

export async function sendEmail(template: string, recipient: string, locals: IEmailLocals): Promise<void> {
  try {
